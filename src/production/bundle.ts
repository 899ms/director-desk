...
import { scenePromptFile, selectedPromptMode } from './prompts.ts';
export async function productionEntries(project: Project, video?: Blob, document?: SceneDocument): Promise<ZipEntry[]> {
    ...
    const promptFiles: { sceneId: string | null; sceneName: string; file: string | null; textOnlyFile: string | null; selectedMode: string }[] = [];
    ...
    for (const [index, scene] of scenes.entries()) {
        const prompt = scenePromptFile(scene.project, scene.name, 'reference-video');
        const file = prompt ? `逐场提示词/${String(index + 1).padStart(4, '0')}-${prompt.name}` : null;
        if (prompt && file) entries.push({ ...prompt, name: file });
        const textPrompt = scenePromptFile(scene.project, scene.name, 'text-only');
        const textOnlyFile = textPrompt ? `逐场提示词/${String(index + 1).padStart(4, '0')}-${textPrompt.name}` : null;
        if (textPrompt && textOnlyFile) entries.push({ ...textPrompt, name: textOnlyFile });
        promptFiles.push({ sceneId: scene.id, sceneName: scene.name, file, textOnlyFile, selectedMode: selectedPromptMode(scene.project.production) });
    }
    ...
    text('提示词素材.txt', ['以下是当前戏段的写作素材，不是已完成的视频生成提示词。完整提示词如已保存，见“逐场提示词”目录。', '', data.fixedPrompt || '（本工程尚未填写固定提示词头）', '',
        ...(selectedPromptMode(data) === 'text-only'
            ? ['当前为纯文本模式，不依赖参考视频。为本段独立写明起始画面、景别、构图、运镜及剧情；人物与场景参考图如已关联，见《素材对应关系.json》。', '仅描述当前镜头内的可见动作；画外对白标明来源，反应镜头需明确切镜。跨段优先改变主体或景别，按情境安排合理情绪。']
            : ['参考《参考视频.mp4》的取景、运镜、切镜、场景布局、人物站位和行走路线。人物身份与参考图对应见《素材对应关系.json》。', '白模动作表达行为意图，人物表演自然完成，避免逐帧复刻僵硬的关节运动。']), '', notesText(project)].join('\n'));
    text('使用说明.txt', `工程：${project.name}\n${document ? "工程文件包含全部独立戏段；视频、参考图、角色对应关系、切镜及剧情备注属于当前戏段。" : "本包包含可编辑工程、参考图、角色对应关系、切镜时间和剧情备注。"}\n逐场提示词目录分别包含各戏段已保存的参考视频及纯文本提示词；未填写的戏段不生成空文件，具体见素材对应关系.json 的 promptFiles（file 为参考视频文稿，textOnlyFile 为纯文本文稿）。提示词是保存的文稿，修改编排后需同步更新。参考视频模式的其他戏段需分别导出对应视频与图片，不能共用当前段视频；纯文本模式不依赖参考视频。\n${video ? '参考视频由同一工程按切镜逐帧导出，为无声白模参考。' : '本次未包含视频，请另外导出参考视频。'}\n剧情备注和提示词素材不会自动烧录为字幕或配音。跨时间段的台词仅在原始备注中保存一次，由后续提示词工作流分配。\n`);
